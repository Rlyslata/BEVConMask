...
def parse_config():
    ...
    cfg.DATA_CONFIG.DATASET_SKIP_STEP = args.data_skip_step
...
def main():
    ...
    args.workers = cfg.OPTIMIZATION.NUM_WORKERS_PER_GPU

    if args.fix_random_seed:
        common_utils.set_random_seed(666 + cfg.LOCAL_RANK)

    output_dir = Path(cfg.SAVE_PATH, args.name, datetime.datetime.today().strftime("%d%m%y-%H%M"))
    ...
    if args.pretrained_model is not None:
        ...
        args.pretrained_model = process_pretrained_model(args.pretrained_model, logger=logger)
        model.load_params_from_file(filename=args.pretrained_model, to_cpu=dist_train, logger=logger)
        # model = load_pretrain_params_from_file(model=model, model_cfg=cfg.MODEL, pretrain_path=args.pretrained_model, to_cpu=dist_train, logger=logger)
    ...
    logger.info('**********************Start evaluation %s**********************' %
                (args.name))
    ...
    logger.info('**********************End evaluation %s**********************' %
                (args.name))

def process_pretrained_model(pretrain_path=None, logger=None):
    if not os.path.isfile(pretrain_path):
        raise FileNotFoundError
    
    logger.info('==> Process pretrained model')

    if pretrain_path.endswith('_converted.pt'):
        logger.info('Model has been converted')
        return pretrain_path
    
    logger.info('Start converting model')
    ckpt = torch.load(pretrain_path)
    if 'model_points' in ckpt:
        logger.info('Convert model from model_points')
        ckpt = ckpt['model_points']
        pretrain_path = pretrain_path.replace('.pt', '_converted.pt')
        torch.save({'model_state': ckpt}, pretrain_path)
    elif 'state_dict' in ckpt:
        logger.info('Convert model from state_dict')
...
def load_pretrain_params_from_file(model, model_cfg, pretrain_path=None, to_cpu=False, logger=None):
    if not os.path.isfile(pretrain_path):
        raise FileNotFoundError
    
    if model_cfg.NAME == 'SECONDNet':
        logger.info('==> Loading SECOND parameters from pretrain model %s to %s' % (pretrain_path, 'CPU' if to_cpu else 'GPU'))
