...
class BaseBEVBackbone(nn.Module):
    ...
    # @profile(stream=open('./CSBEV/output/memory/BaseBEVBackbone_forward.log', 'w+'))
...
class VoxelBackBone8x(nn.Module):
    ...
    # @profile(stream=open('./CSBEV/output/memory/VoxelBackBone8x_forward.log', 'w+'))
...
class HeightCompression(nn.Module):
    ...
    # @profile(stream=open('./CSBEV/output/memory/HeightCompression_forward.log', 'w+'))
...
class SECOND(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.is_final = kwargs['config'].POINT_ENCODER.FINAL
        if self.is_final:
            self.final = nn.Conv2d(512, kwargs['config'].POINT_ENCODER.FEATURE_DIMENSION, kernel_size=1)
    
    # @profile(stream=open('./CSBEV/output/memory/second_forward.log', 'w+'))
    def forward(self, voxels, coordinates):
        with torch.no_grad():
            # bs = coordinates[-1, 0].item() + 1
            bs = torch.max(coordinates[:, 0]).item() + 1
